...
function renderAnnualDayOfWeekChart(metrics) {
  ...
  // Safe chart destruction
  if (window.annualDayOfWeekChart && typeof window.annualDayOfWeekChart.destroy === "function") {
    try {
  ...
  window.annualDayOfWeekChart = null;

  // Initialize day counters: [Monday, Tuesday, Wednesday, Thursday, Friday]
  const dayOfWeekStats = [0, 0, 0, 0, 0];
  const currentYear = metrics.year || new Date().getFullYear();

  // FIXED: Use filtered members if individual view
  const targetMembers = metrics.isFiltered ? [metrics.selectedMember] : teamMembers;
  ...
  for (let month = 0; month < 12; month++) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      // Count leave for target members only
      targetMembers.forEach(member => {
        if (hasLeaveOnDay(member, monthKey, day)) {
          dayOfWeekStats[dayOfWeek - 1]++; // Convert to 0-based (Monday=0)
        }
      });
    }
  }
...
function renderAnnualLeaveDurationChart(metrics) {
  ...
  // Safe chart destruction
  if (window.annualLeaveDurationChart && typeof window.annualLeaveDurationChart.destroy === "function") {
    try {
  ...
  window.annualLeaveDurationChart = null;
  ...
  // Duration categories
  const durationCounts = [0, 0, 0, 0]; // [1 day, 2-3 days, 4-5 days, 6+ days]
  const durationLabels = ["1 day", "2-3 days", "4-5 days", "6+ days"];

  // Analyze leave blocks for target members
  targetMembers.forEach(member => {
    for (let month = 0; month < 12; month++) {
      const monthDate = new Date(currentYear, month, 1);
      const monthKey = getKey(monthDate);
      ...
      // Get days that have any leave (full, morning, or afternoon)
      const leaveDays = Object.keys(memberData)
        .filter(day => memberData[day]) // Only days with leave data
        .map(d => parseInt(d))
        .sort((a, b) => a - b);
      ...
      for (let i = 1; i < leaveDays.length; i++) {
        if (leaveDays[i] === leaveDays[i - 1] + 1) {
          ...
          leaveBlocks.push(currentGroup.length);
      ...
      leaveBlocks.forEach(blockSize => {
        if (blockSize === 1) {
          durationCounts[0]++; // 1 day
        } else if (blockSize <= 3) {
          durationCounts[1]++; // 2-3 days
        } else if (blockSize <= 5) {
          durationCounts[2]++; // 4-5 days
        } else {
          durationCounts[3]++; // 6+ days
        }
      });
    }
  });
      ...
      options: {
        ...
        scales: {
          y: {
            beginAtZero: true,
            stepSize: 1,
            title: { display: true, text: "Number of Leave Blocks" },
          },
        },
      },
    });

    console.log("Duration chart data:", durationCounts);
  } catch (error) {
...
// Make annual dashboard functions globally accessible
window.renderAnnualDashboard = renderAnnualDashboard;
window.calculateAnnualMetrics = calculateAnnualMetrics;
window.renderAnnualKPICards = renderAnnualKPICards;
window.renderAnnualCharts = renderAnnualCharts;
window.populateAnnualMemberFilter = populateAnnualMemberFilter;
window.onAnnualMemberFilterChange = onAnnualMemberFilterChange;
window.setAnnualMemberFilter = setAnnualMemberFilter;
window.resetAnnualMemberFilter = resetAnnualMemberFilter;

window.renderAnnualMonthlyTrend = renderAnnualMonthlyTrend;
window.renderAnnualMemberComparison = renderAnnualMemberComparison;
window.renderAnnualQuarterlyChart = renderAnnualQuarterlyChart;
window.renderAnnualLeavePattern = renderAnnualLeavePattern;
window.renderAnnualDayOfWeekChart = renderAnnualDayOfWeekChart;
window.renderAnnualLeaveDurationChart = renderAnnualLeaveDurationChart;
