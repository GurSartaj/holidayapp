...
function addCalendarEventHandlers(dayElement, date) {
  ...
  dayElement.addEventListener("click", (e) => {
    if (e.detail === 1) {
      ...
      const categoryDropdown = document.getElementById("leaveCategoryDropdown");
      const selectedCategory = categoryDropdown
      ...
      if (savedHoliday || window.pendingHolidays[pendingKey][dayNum]) {
        ...
        applyHalfDayVisuals(dayElement, "none", "PTO");
      } else {
        // Add new full day holiday (SIMPLE STRING)
        dayElement.classList.add(
          "holiday",
          "full-day",
          `category-${selectedCategory.toLowerCase()}`
        );
        // Store type and category in ONE string: "full|Training"
        window.pendingHolidays[pendingKey][dayNum] = `full|${selectedCategory}`;

        applyHalfDayVisuals(dayElement, "full", selectedCategory);
      }
  ...
  dayElement.addEventListener("contextmenu", (e) => {
    ...
    // Get selected category from dropdown
    const categoryDropdown = document.getElementById("leaveCategoryDropdown");
    const selectedCategory = categoryDropdown ? categoryDropdown.value : "PTO";
    ...
    const currentLeaveData =
      window.pendingHolidays[pendingKey][dayNum] || savedHoliday;

    // Parse the type from "type|category" format
    const currentLeaveType = currentLeaveData
      ? currentLeaveData.split("|")[0]
      : null;
    ...
    if (currentLeaveType === "morning") {
      ...
      window.pendingHolidays[pendingKey][
        dayNum
      ] = `afternoon|${selectedCategory}`;
      applyHalfDayVisuals(dayElement, "afternoon", selectedCategory);
      ...
      // Set to morning (from none or full)
      dayElement.classList.add(
        "morning-half",
        `category-${selectedCategory.toLowerCase()}`
      );
      window.pendingHolidays[pendingKey][
        dayNum
      ] = `morning|${selectedCategory}`;
      applyHalfDayVisuals(dayElement, "morning", selectedCategory);
...
function applyHalfDayVisuals(cell, leaveType, category = "PTO") {
  // Get category color
  ...
  const leaveColor = categoryColors[category] || "#dc3545";
  let borderColor = leaveColor;

  // Override with public holiday border if applicable
  ...
  if (leaveType === "morning") {
    cell.style.setProperty(
      "background",
      `linear-gradient(180deg, ${leaveColor} 0%, ${leaveColor} 50%, transparent 50%, transparent 100%)`,
      "important"
    );
    ...
    cell.style.setProperty(
      "background",
      `linear-gradient(180deg, transparent 0%, transparent 50%, ${leaveColor} 50%, ${leaveColor} 100%)`,
      "important"
    );
    cell.style.setProperty("border", `3px solid ${borderColor}`, "important");
    cell.style.setProperty("color", "white", "important");
    ...
    cell.style.setProperty("background", leaveColor, "important");
    cell.style.setProperty("border", `3px solid ${borderColor}`, "important");
    cell.style.setProperty("color", "white", "important");
    cell.style.setProperty("font-weight", "bold", "important");
  } else {
...
function loadUserHolidays() {
  ...
  calendarGrid.querySelectorAll(".calendar-day").forEach((cell) => {
    ...
    // Check pending changes first, then saved data (SIMPLE STRINGS)
    const leaveData = pendingData[day] || userData[day];

    if (leaveData) {
      // Parse "type|category" format
      const [leaveType, category = "PTO"] = leaveData.split("|");

      // Add category class
      cell.classList.add(`category-${category.toLowerCase()}`);

      if (leaveType === "morning") {
        cell.classList.add("morning-half");
        applyHalfDayVisuals(cell, "morning", category);
      } else if (leaveType === "afternoon") {
        cell.classList.add("afternoon-half");
        applyHalfDayVisuals(cell, "afternoon", category);
      } else if (leaveType === "full") {
        cell.classList.add("holiday", "full-day");
        applyHalfDayVisuals(cell, "full", category);
      }
    } else {
      applyHalfDayVisuals(cell, "none", "PTO");
    }
  });
}
...
async function saveHolidays() {
  ...
  // Get pending changes (already in "type|category" format)
  const pendingData = window.pendingHolidays[pendingKey] || {};

  // Apply pending changes to saved data - SIMPLE STRINGS!
  ...
  try {
    // Force save with localStorage backup immediately
    saveToLocalStorage();

    // Then sync to Google Sheets
    await saveToGoogleSheets("holidays", holidayData);

    console.log("✅ Saved to Google Sheets successfully");
  } catch (error) {
    console.error("❌ Google Sheets save failed:", error);
    showMessage("⚠️ Saved locally only - Google Sheets sync failed", "warning");
  }
...
function calculateMemberHolidays(member, year, month) {
  ...
  Object.keys(monthData).forEach((day) => {
    const leaveData = monthData[day]; // "full|Training" or just "full"
...
window.getAllHolidaysForDate = getAllHolidaysForDate;
